...
def main():
    ...
    parser.add_argument("--persist_dir", type=str, default="docs_faiss")
    args = parser.parse_args()

    print(f"Data Directory :{args.docs_dir}")
    print(f"Vector Index  Path :{args.persist_dir}")

    embedding = SentenceTransformerEmbeddings(model_name="all-mpnet-base-v2")
    print(f"Emberdding: {embedding.model_name}")

    if os.path.exists(args.persist_dir):
        print(f"Loading FAISS index from : {args.persist_dir}")
        vectorstore = FAISS.load_local(args.persist_dir, embeddings=embedding)
        print("done")
    else:
        ...
        vectorstore = FAISS.from_documents(frags, embedding=embedding)
